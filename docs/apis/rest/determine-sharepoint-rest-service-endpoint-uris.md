...
Здесь вы можете создать более конкретные конечные точки REST, "обходя" объектную модель, с использованием имен API клиентской объектной модели, разделенных знаком косой черты (/). В приведенной ниже таблице есть примеры аналогичной конечной точки REST и вызовов клиентской объектной модели. 
