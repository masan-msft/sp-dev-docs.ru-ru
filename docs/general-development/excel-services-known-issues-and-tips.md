...
---
# <a name="excel-services-known-issues-and-tips"></a>Известные проблемы и советы по службам Excel

The following are a known issues and tips for working with Службы Excel.
...
You can view the Веб-службы Excel Web Services Description Language (WSDL) page by navigating to the following URL on the server:  `http://<server>/<customsite>/_vti_bin/excelservice.asmx?WSDL`
...
For more information, see  [Доступ к API SOAP](accessing-the-soap-api.md).
...
- For the rest of the Веб-службы Excel methods, besides credentials, the caller needs "view" permission (in SharePoint Foundation) or "read" permission (in a file share) for the workbook.
...
When Microsoft Visual Studio creates a proxy class for a client project that calls Веб-службы Excel, it has the following behavior: 
...
The affected Веб-службы Excel methods are:
...
Службы Excel does not maintain the original order of strings in a workbook shared-string table (the Sharedstrings.xml part within the Формат XML Microsoft Office Excel file). For example, execute the following steps:
...
4. Open the file in the document library by using Веб-клиент Excel.
