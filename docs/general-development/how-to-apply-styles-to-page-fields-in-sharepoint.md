...
10. На сопоставленном сетевом диске на компьютере откройте макет HTML-страницы в редакторе HTML.
    ...
    > [!NOTE]
    > Дополнительные сведения см. в статье [Инструкции. Сопоставление сетевого диска с коллекцией эталонных страниц SharePoint](how-to-map-a-network-drive-to-the-sharepoint-master-page-gallery.md). 
...
13. На сервере перейдите к следующей папке и откройте HtmlEditorStyles.css:
    ...
    > [!IMPORTANT]
    > Не меняйте используемую по умолчанию таблицу стилей HtmlEditorStyles.css. Она предоставляет стили для каждого объекта RichHtmlField в ферме. Кроме того, при обновлении служб или установке новой версии этот файл может быть перезаписан, что приведет к потере всех изменений. 
