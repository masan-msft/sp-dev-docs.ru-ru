...
Можно выполнять базовые создания, чтения, обновления и удаления (CRUD) операции с помощью интерфейса REST, доступного в SharePoint. REST предоставляет всех объектов SharePoint и операции, доступные в другие клиентские интерфейсы API SharePoint. Одно из преимуществ использования REST состоит в том, что вам не требуется добавлять ссылки на библиотеки и клиентские сборки SharePoint. Вместо этого соответствующим конечным точкам отправляются HTTP-запросы на получение или обновление сущностей SharePoint, таких как веб-сайты, списки и элементы списков. Подробное введение в интерфейс SharePoint REST и ее архитектуры в разделе [операции запросов OData используется в запросах SharePoint REST](http://msdn.microsoft.com/library/d4b5c277-ed50-420c-8a9b-860342284b72%28Office.15%29.aspx).
