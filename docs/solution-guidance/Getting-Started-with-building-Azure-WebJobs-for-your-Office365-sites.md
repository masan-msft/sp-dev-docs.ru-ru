...
> [!NOTE] 
> В образце по завершении работы будет использоваться учетная запись + пароль подход (например, учетная запись службы). Мы обсудим проверки подлинности, параметры, расположенным ниже в статье извлечения и возврата ссылки на другие возможности.
...
> [!IMPORTANT] 
> На этом этапе альтернатива «Для выполнения» только обеспечивает «По требованию» или «Продолжить», но скоро будет поддержка «Расписание» также – которого — это действительно необходимо.
...
> [!NOTE] 
> Эти диалоговые окна может немного отличаться, если вы используете более ранней версии Visual Studio. Кроме того я уже вошел, если вы выполняете это в первый раз появляется диалоговое окно входа для входа в учетную запись Azure. Это связано с подготовкой.
