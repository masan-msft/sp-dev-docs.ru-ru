...
4. Создайте пространство имен шины Azure службы с поддержкой службы ACS.
    
    1. Установка Azure PowerShell. Дополнительные сведения можно [как: Установка Azure PowerShell](http://azure.microsoft.com/en-us/documentation/articles/powershell-install-configure/#Install).
    
    2. Запустите **Azure PowerShell**.
    
    3. Введите **Добавить AzureAccount**.
    
    4. Введите свой адрес электронной почты в диалоговом окне **входа в Windows Azure** и нажмите кнопку **Продолжить**.
    
    5. Введите свой пароль и нажмите кнопку **Вход**.
    
    6. Создайте новое пространство имен шины службы с помощью следующего командлета PowerShell.
       ...
       -  _Имя_пространства_имен_ — это имя пространства имен шины служб Azure.
    
       -  _Область_ — региона, ближайшего к вам. Например можно указать **«Запад США»**. Необходимо включить имя регион в двойные кавычки.
    
    7. Вернуться к вашей портала управления Azure. Выберите **СЛУЖЕБНОЙ ШИНЫ**и нажмите кнопку введенное имя пространства имен.
    
    8. Выберите **Управление строки подключения**и нажмите кнопку Копировать в **СТРОКУ подключения службы ACS**.
    
    9. Вернуться в Visual Studio.
    
    10. Щелкните правой кнопкой мыши Core.EventReceivers > **Свойства** > **SharePoint**.
    
    11. Установите флажок **Включить отладку через шину службы Microsoft Azure**.
    
    12. В поле **Строка подключения шины службы Microsoft Azure**вставьте строку подключения службы ACS.
...
**Примечание**  В окне Свойства проекта Core.EventReceiver доступны только свойства, **Управление установленными приложениями** и **Обработка удаляемого приложения** . В этом примере кода показано, как добавить обработчик событий **ItemAdded** для списка на веб-сайт с помощью события **AppInstalled** во время установки надстройки.
