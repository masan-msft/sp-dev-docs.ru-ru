...
> [!NOTE] 
> Расширенный PnP решение с десяти отдельных примеры задания таймера из примеры задания фактический истечения срока действия содержимого, «Hello world» в разделе https://github.com/SharePoint/PnP/tree/dev/Solutions/Core.TimerJobs.Samples
...
- Добавление существующего проекта PnP основных источника в проект. Это позволит выполнить пошагово код PnP основных при отладке. 

    > [!NOTE] 
    > Ответственность за поддержание этот код, обновлены с последними изменениями, добавляемого PnP будет.
...
> [!NOTE] 
> Подробная информация о развертывании WebJob Azure в разделе [Приступая к работе с azure WebJobs («задания таймера») для сайтов Office 365](https://github.com/SharePoint/PnP-Guidance/blob/master/articles/Getting-Started-with-building-Azure-WebJobs-for-your-Office365-sites.md). 
...
> [!NOTE] 
> Определенные веб-сайтов разрешения логику не работы с проверкой подлинности только для приложений. Подробные сведения можно найти в следующем разделе. 
...
> [!NOTE] 
> При использовании только логику задание таймера не удается при использовании API-интерфейсы, которые не работают с **AuthenticationType.AppOnly**. Типичные примеры — это API поиска, записи в хранилище данных таксономии и с помощью API профилей пользователей.
...
> [!NOTE] 
> Необходимо использовать `ExecuteQueryRetry` в коде реализации задания таймера.
