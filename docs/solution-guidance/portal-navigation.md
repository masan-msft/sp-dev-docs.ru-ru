...
> [!NOTE] 
> Навигация на основе поиска имеет зависимости от индекса поиска. SharePoint непрерывно обходит контент портала; Тем не менее по-прежнему будет небольшая задержка перед изменений в списке SharePoint отображаются в индексе поиска.
