...
> [!NOTE] 
> Чтобы просмотреть видеозапись с пошаговой создания и сохранения и применение шаблона подготовки, перейдите к [Приступая к работе с PnP модуля подготовки](https://channel9.msdn.com/blogs/OfficeDevPnP/Getting-Started-with-PnP-Provisioning-Engine).
...
> [!NOTE] 
> Репозиторий команды SharePointPnP.PowerShell содержит три версии MSI команд Windows PowerShell. Два являются для локального использования (по одному для SharePoint 2013), а другое для SharePoint 2016, а третье — для SharePoint Online.
