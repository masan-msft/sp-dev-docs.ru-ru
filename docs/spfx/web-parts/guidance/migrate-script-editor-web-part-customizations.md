...
> Примечание. Сведения в этой статье применимы к модификациям, созданным с помощью веб-частей редактора содержимого и редактора скриптов. Любое упоминание веб-части редактора скриптов следует толковать как _веб-часть редактора содержимого и веб-часть редактора скриптов_.
...
Модификации SharePoint, созданные с помощью веб-части редактора скриптов, обычно состоят из разметки HTML, включенной в веб-часть, и одной или нескольких ссылок на файлы JavaScript. При преобразовании существующих модификаций в решение SharePoint Framework, скорее всего, потребуется переместить разметку HTML из веб-части редактора скриптов в метод **render** клиентской веб-части SharePoint Framework. Ссылки на внешние сценарии будут заменены ссылками в свойстве **externals** из файла **config.json**. Необходимо скопировать внутренние скрипты в папку исходного кода веб-части, а затем сослаться на нее из веб-части с помощью операторов **require()**.
