...
По указанным выше причинам пользовательские решения пока не могут безопасно использовать компоненты Fabric. С другой стороны, мы понимаем, что Fabric Core и Fabric React необходимы разработчикам для создания решений. Мы постараемся устранить эту проблему как можно скорее.
...
## <a name="using-office-ui-fabric-with-non-react-based-web-parts"></a>Использование Office UI Fabric с веб-частями не на React

В настоящее время этот сценарий не поддерживается, а с возможными реализациями могут возникнуть непредвиденные проблемы после выпуска новой версии Office UI Fabric. Мы разрабатываем надежный механизм интеграции для **Office UI Fabric Core**, **fabric.component.css** и **веб-частей не на React**. 
...
Все стили в CSS-файлах с ограниченной областью действия включены в селектор потомков, например ms-Fabric-core--v6 ms-Icon--list. Во время компиляции инструменты получат версию Office UI Fabric Core, с помощью которой была создана веб-часть. Это может быть версия, входящая в состав SPFx. Кроме того, разработчики веб-частей могут явно указывать зависимость от определенной версии Office UI Fabric Core в файле package.json.

Разделителю веб-части будет назначена эта область действия, т. е. `<div data-sp-webpart class="ms-Fabric-core--v6">`. Платформа загрузит определенную основную версию CSS-файла с областью действия на уровне Fabric Core. Если веб-часть создана с помощью CSS-файла Fabric Core версии 6.0.0, платформа скачает файл fabric-6-scoped.css во время загрузки веб-части.

Остальная часть страницы будет содержать Office UI Fabric Core с неограниченной областью действия. Таким образом, согласно правилам специфичности CSS внутри разделителя веб-части имеют приоритет CSS с ограниченной областью действия. Веб-часть и ее содержимое будут адаптированы к определенной версии Office UI Fabric Core, выбранной разработчиком.
...
**Отсутствие надежной поддержки вложения.** Этот подход работает, только если страница и веб-части Майкрософт используют версию Office UI Fabric Core (т. е. ms-Icon--list) с неограниченной областью действия, а сторонние веб-части — только CSS Office UI Fabric Core с неограниченной областью действия, как описано выше. Причина заключается в том, что CSS с ограниченной областью действия, примененный к веб-части, переопределяет CSS с неограниченной областью действия на странице. Обратите внимание: как описывалось ранее, если специфичность двух классов CSS совпадает, то приоритет имеет класс, загруженный позже. Следовательно, для обеспечения согласованности интерфейса важно, чтобы специфичность класса CSS с ограниченной областью действия была выше.
