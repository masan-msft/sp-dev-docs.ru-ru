...
Решения SharePoint Framework развертываются с помощью **SPPKG**-файла, установленного в каталоге приложений в клиенте SharePoint. **SPPKG**-файл похож на пакет надстройки SharePoint и подчиняется тем же соглашениям, касающимся управления версиями. Текущая версия **SPPKG**-пакета определяется с помощью состоящего из четырех частей (ОСНОВНОЙ_НОМЕР_ВЕРСИИ.ДОПОЛНИТЕЛЬНЫЙ_НОМЕР_ВЕРСИИ.НОМЕР_РЕДАКЦИИ.НОМЕР_СБОРКИ) номера, хранящегося в файле **config/package-solution.json**. Для простоты разработчикам следует синхронизировать этот номер с номером версии в файле **package.json**, так как оба номера относятся к версии проекта в целом.
