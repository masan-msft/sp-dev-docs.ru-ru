...
Трудности с глобальными стилями CSS хорошо описаны в следующей презентации на примере React и JavaScript: [React CSS в JS](https://speakerdeck.com/vjeux/react-css-in-js).
...
* [Office UI Fabric Core](https://developer.microsoft.com/ru-RU/fabric). Набор, включающий основные стили, оформление, адаптируемую сетку, анимацию, значки и другие стандартные блоки общего языка дизайна.

* [Office UI Fabric React](https://developer.microsoft.com/ru-RU/fabric#/components). Набор компонентов React, основанных на языке дизайна Fabric, для использования в проектах на основе React.

## <a name="office-ui-fabric-core-package"></a>Пакет Office UI Fabric Core

Пакет npm с SharePoint Framework Fabric Core (sp-office-ui-fabric-core) содержит подмножество поддерживаемых стилей Fabric Core, которые можно безопасно использовать в компоненте SharePoint Framework. 
...
Чтобы использовать пакет Fabric Core в имеющемся проекте, установите пакет в качестве зависимости для разработки:
...
### <a name="use-fabric-core-styles"></a>Использование стилей Fabric Core

Чтобы использовать стили Fabric Core, необходимо импортировать объявления SPFabricCore в файл SASS.

> [!NOTE] 
> Убедитесь, что установлен пакет npm sp-office-ui-fabric-core.
...
Рекомендуем установить последнюю версию пакета Office UI Fabric React и добавить в пакет явную зависимость от этой конкретной версии. В пакет будут включены компоненты, используемые в решении SharePoint Framework — веб-части или расширении (в зависимости от того, для каких целей используются компоненты Fabric React). 
...
## <a name="the-css-challenge-with-office-ui-fabric"></a>Трудности с CSS в Office UI Fabric
...
### <a name="global-css-styles"></a>Глобальные стилей CSS
...
Как избегать использования глобальных стилей CSS любой ценой — это серьезная проблема. На данный момент библиотеки Fabric Core и Fabric React содержат глобальные стили. Отсутствие встроенных в браузеры решений для управления областями стилей делает это очень трудной задачей.
  ...
  - Суть проблемы хорошо раскрыта в обсуждении [React CSS в JS](https://speakerdeck.com/vjeux/react-css-in-js). 
  
В Интернете есть множество другой документации, посвященной решениям проблемы с глобальными пространствами имен.

### <a name="css-specificity"></a>Специфичность CSS

Как [правила CSS](https://developer.mozilla.org/ru-RU/docs/Web/CSS/Specificity) применяются к веб-интерфейсу? Стили с большей специфичностью переопределяют стили с меньшей специфичностью. Но самое важное — понять, как применяются правила специфичности. Общий порядок приоритетов (по убыванию) выглядит так:
...
### <a name="shortcomings-of-this-strategy"></a>Недостатки этой стратегии
...
#### <a name="lack-of-reliable-nesting-support"></a>Отсутствие надежной поддержки вложения
...
Кроме того, несколько расширений, вложенных друг в друга, не могут использовать разные версии Fabric Core. Например, в примере ниже применяется только `ms-Fabric-core--v6`.
...
#### <a name="leakage-from-unscoped-classes"></a>Утечки из неограниченных классов
