...
## <a name="create-a-button-to-launch-a-flow"></a>Создание кнопки для запуска потока
...
С помощью форматирования столбцов вы можете создавать кнопки, при нажатии которых запускаются потоки для соответствующих элементов списка.  Если поток настроен на сбор данных у конечного пользователя перед запуском, то после нажатия кнопки появится панель запуска потока.  В противном случае поток просто запустится.

Чтобы использовать приведенный ниже пример, необходимо указать ИД нужного потока.  Этот ИД хранится в атрибуте `customRowAction` элемента `button`.  Чтобы получить ИД потока, сделайте следующее:

1. Нажмите "Поток" > "Просмотреть ваши потоки" в том списке SharePoint, где настроен поток.
2. Выберите нужный поток
3. Скопируйте ИД из окончания URL-адреса.  
